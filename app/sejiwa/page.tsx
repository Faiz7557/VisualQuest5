...
export default function SejiwaDemoPage() {
  const [mode, setMode] = useState<"web" | "sms">("web");
        ...
        <div className="flex items-center gap-3">
          <div className="inline-flex rounded-xl bg-[#0b162a] p-1 border border-white/10 text-xs">
            ...
            >
              ...
              <span>Mode SMS/USSD (3T Offline)</span>
            </button>
          </div>
        </div>
      </div>

      {/* 2. Chat UI & Live Triage Dashboard Split */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Chat Interface Column */}
        <div className="lg:col-span-7 flex flex-col h-[650px] glass-card rounded-2xl border border-white/10 overflow-hidden">
          {/* Chat Header */}
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-sm text-white font-heading">
                    {mode === "web" ? "SEJIWA+ Intelligent Assistant" : "SMS Gateway 119 (Tanpa Internet)"}
                  </h3>
                  <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                </div>
                <p className="text-[10px] text-slate-400">
                  {mode === "web" ? "Hotline Kemenkes 119 ext 8 • Enkripsi End-to-End" : "Koneksi Jaringan Seluler Dasar (2G/3G)"}
                </p>
              </div>
            </div>
                    ...
                    <div
                      className={`rounded-2xl p-3.5 text-xs sm:text-sm leading-relaxed ${
                        isUser
                          ...
                          : mode === "sms"
                          ? "bg-[#0b162a] text-emerald-300 font-mono border border-emerald-500/20 rounded-tl-sm"
          ...
          >
            <input
              type="text"
              placeholder={mode === "web" ? "Ketikkan apa yang kamu rasakan saat ini..." : "Balas SMS / Ketik USSD *119#..."}
              value={input}
        ...
        {/* Live Triage Evaluation Engine Column */}
